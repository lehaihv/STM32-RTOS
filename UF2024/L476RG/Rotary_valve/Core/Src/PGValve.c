        ...
        if (HAL_UART_Receive(hvalve->huart, &tmp, 1, 10) == HAL_OK) {
            if (tmp == PGV_FRAME_STX) {
                ...
                for (int i = 1; i < PGV_FRAME_SIZE; ++i) {
                    if (HAL_UART_Receive(hvalve->huart, &resp[i], 1, timeout) != HAL_OK)
                        return 4; // timeout
                }
